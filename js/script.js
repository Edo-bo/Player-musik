...
async function playSong(index) {
    ...
    try {
        ...
        // <-- UPDATED: Direct access to download URL sesuai struktur baru (data.result.mp3)
        const downloadUrl = data && data.result && data.result.mp3 ? data.result.mp3 : null;
...
function downloadCurrentSong() {
    ...
    fetch(`${API_URL.DOWNLOAD_MP3}?url=${encodeURIComponent(song.videoUrl)}`)
        ...
        .then(data => {
            ...
            // <-- UPDATED: Sesuaikan path data download (data.result.mp3)
            const downloadUrl = data && data.result && data.result.mp3 ? data.result.mp3 : null;
